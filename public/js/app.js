...
const App = {
  ...
  async renderPresetsView(container) {
    ...
    try {
      ...
      container.innerHTML = '<div class="presets-view"><div class="presets-header"><div><h1>Пресеты</h1><p class="presets-description">Пресеты — это готовые конфигурации (txt-файлы) для Zapret GUI, которыми пользователи делятся друг с другом. Каждый пресет содержит настройки обхода блокировок для определённых сайтов, провайдеров или сценариев. Вы можете скачать подходящий пресет и импортировать его в свою копию Zapret GUI, а также поделиться собственным пресетом с сообществом.</p></div><button class="btn btn-primary" id="create-preset-btn">+ Опубликовать пресет</button></div><div class="presets-toolbar"><input type="text" class="form-input" id="presets-search" placeholder="Поиск по названию или описанию..." value="" style="max-width:350px"><select class="form-select" id="presets-sort" style="max-width:200px"><option value="newest">Сначала новые</option><option value="oldest">Сначала старые</option><option value="most_downloaded">По скачиваниям</option><option value="most_commented">По комментариям</option></select></div><div class="presets-list" id="presets-list">' + (data.presets.length > 0 ? presetCards : '<div class="empty-state"><h3>Пресетов пока нет</h3><p>Будьте первым, кто опубликует свой пресет!</p></div>') + '</div>' + (data.total > data.limit ? '<div class="pagination">' + (data.page > 1 ? '<button class="btn btn-sm" data-presets-page="' + (data.page - 1) + '">&larr; Назад</button>' : '') + '<span>Страница ' + data.page + ' из ' + Math.ceil(data.total / data.limit) + '</span>' + (data.page < Math.ceil(data.total / data.limit) ? '<button class="btn btn-sm" data-presets-page="' + (data.page + 1) + '">Далее &rarr;</button>' : '') + '</div>' : '') + '</div>';
...
(function() {
  ...
  overlay.innerHTML =
    ...
    '<div class="lightbox-zoom-controls">' +
      '<button class="lightbox-zoom-btn" data-action="out" title="Zoom out">&minus;</button>' +
      '<span class="lightbox-zoom-level">100%</span>' +
      '<button class="lightbox-zoom-btn" data-action="in" title="Zoom in">&plus;</button>' +
      '<button class="lightbox-zoom-btn lightbox-zoom-reset" data-action="reset" title="Reset zoom">&#8634;</button>' +
    '</div>' +
  ...
  const zoomLevelEl = overlay.querySelector('.lightbox-zoom-level');

  // Zoom state
  const MIN_ZOOM = 0.25;
  const MAX_ZOOM = 10;
  const ZOOM_STEP = 0.15; // multiplicative step per wheel tick
  let scale = 1;
  let panX = 0;
  let panY = 0;
  ...
  function clampPan() {
    ...
    const imgW = lbImg.naturalWidth ? Math.min(lbImg.offsetWidth * scale, rect.width * scale) : rect.width * scale;
    const imgH = lbImg.naturalHeight ? Math.min(lbImg.offsetHeight * scale, rect.height * scale) : rect.height * scale;
    const maxPanX = Math.max(0, (lbImg.offsetWidth * scale - rect.width) / 2);
    const maxPanY = Math.max(0, (lbImg.offsetHeight * scale - rect.height) / 2);
  ...
  wrapper.addEventListener('wheel', (e) => {
    ...
    const newScale = scale * (1 + direction * ZOOM_STEP);
  ...
  overlay.addEventListener('click', (e) => {
    const btn = e.target.closest('.lightbox-zoom-btn');
    if (btn) {
      e.stopPropagation();
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === '+' || e.key === '=') {
      e.preventDefault();
      const rect = lbImg.getBoundingClientRect();
      zoomAtPoint(scale * 1.3, rect.left + rect.width / 2, rect.top + rect.height / 2);
    }
    if (e.key === '-') {
      e.preventDefault();
      const rect = lbImg.getBoundingClientRect();
